...
# town (the NEAREST station) — otherwise the left join fans out and inflates
# the row count. Normalise the join key to UPPERCASE to match HDB's format.
mrt_join_cols = (
...
hdb_filled = hdb_enriched.with_columns(
    pl.col("distance_to_mrt_km").fill_null(99.0).alias("distance_to_mrt_filled"),
)
...
hdb_complete = hdb_enriched.drop_nulls(subset=["distance_to_mrt_km", "nearest_mrt"])
...
hdb_enriched = hdb_enriched.with_columns(
    pl.col("distance_to_mrt_km")
    # skip_nulls=False so null distances (unmatched towns) are passed to the
    # classifier and become "unknown" rather than staying null.
    .map_elements(
        classify_mrt_proximity, return_dtype=pl.String, skip_nulls=False
    ).alias("mrt_proximity"),
    pl.col("school_count")
