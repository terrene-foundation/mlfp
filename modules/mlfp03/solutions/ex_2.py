...
# ── Checkpoint 1 ─────────────────────────────────────────────────────
# degree=1 should underfit (high test MSE), degree=20 should overfit
# (very low train MSE but high test MSE)
...
# ── Checkpoint 2 ─────────────────────────────────────────────────────
assert bv_results[1]["bias_sq"] > bv_results[10]["bias_sq"], \
    "Degree=1 should have higher bias than degree=10"
assert bv_results[1]["variance"] < bv_results[15]["variance"], \
...
assert ridge_results[1.0]["n_zero"] <= 2, \
    "Ridge should produce very few zero coefficients (unlike Lasso)"
...
# ── Checkpoint 4 ─────────────────────────────────────────────────────
# High alpha Lasso should zero out many coefficients
...
# the ones most predictive of the target. Compare which features
# Lasso keeps to what domain knowledge says should matter.
...
# ── Checkpoint 5 ─────────────────────────────────────────────────────
# Higher l1_ratio should produce more zero coefficients (more Lasso-like)
...
# This matters in practice: credit score and payment history are correlated,
# and you want both in your model for interpretability.
...
# Verify that OLS coefficients are generally larger (less regularised)
...
# credit default is driven by a few large effects (credit score, income),
# you might prefer Lasso (Laplace prior — sparse). If you believe many
# small effects contribute equally, Ridge (Gaussian prior — smooth) fits better.
...
print("""
  ...
  churn data. You'll learn when to choose each algorithm and why.
""")
