...
# Manual SVD on standardised data
...
n_95 = int(np.searchsorted(cumulative_evr, 0.95) + 1)
n_90 = int(np.searchsorted(cumulative_evr, 0.90) + 1)
n_80 = int(np.searchsorted(cumulative_evr, 0.80) + 1)
...
for kernel_name, kernel_params in [
    ...
    t0 = time.time()
    kpca = KernelPCA(
        n_components=min(n_for_embedding, 10),
        kernel=kernel_name,
        random_state=42,
        **kernel_params,
    )
...
if UMAP_AVAILABLE:
    ...
    for cfg in umap_configs:
        ...
        # Fit on subsample, transform full dataset (out-of-sample)
        reducer.fit(X_pca[idx_tsne])
        embedding_full = reducer.transform(X_pca)
        elapsed = time.time() - t0
...
X_iso = iso.fit_transform(X_pca[idx_tsne[:n_iso]])
...
intrinsic_dim_nn = estimate_intrinsic_dim_nn(X, k_values=[5, 10, 20, 30])
