...
comparison_df = pl.DataFrame(
    {
        "Policy": ["Random"] * N_EVAL + ["DQN"] * N_EVAL + ["PPO"] * N_EVAL,
        "Evaluation Return": random_returns + dqn_eval_returns + ppo_eval_returns,
    }
)
fig1 = viz.box_plot(comparison_df, "Evaluation Return", group_by="Policy")
...
dqn_cumulative_steps = np.cumsum(
    [max(10, r) for r in dqn_rewards]  # approximate: reward ~ steps in CartPole
).tolist()
...
fig2.add_trace(
    go.Scatter(
        x=dqn_cumulative_steps,
        y=moving_average(dqn_rewards, 20),
        name="DQN (avg 20 episodes)",
        line=dict(color="blue"),
    )
)
fig2.add_trace(
    go.Scatter(
        x=ppo_cumulative_steps,
        y=ppo_returns,
        name="PPO (avg per iteration)",
        line=dict(color="red"),
    )
)
fig2.add_hline(
    y=float(np.mean(random_returns)),
    line_dash="dash",
    line_color="gray",
    annotation_text="Random baseline",
)
fig2.update_layout(
    title="Sample Efficiency: Reward vs Environment Interactions",
    xaxis_title="Total Environment Steps",
    yaxis_title="Average Reward / Return",
)
...
fig3 = go.Figure(
    data=[
        go.Bar(
            x=["DQN", "PPO"],
            y=[dqn_time, ppo_time],
            text=[f"{dqn_time:.1f}s", f"{ppo_time:.1f}s"],
            textposition="auto",
...
fig3.update_layout(
    title="Wall-Clock Training Time (CartPole-v1)",
    yaxis_title="Seconds",
)
...
# Final policy quality
means = [
    float(np.mean(random_returns)),
    float(np.mean(dqn_eval_returns)),
    float(np.mean(ppo_eval_returns)),
]
stds = [
    float(np.std(random_returns)),
    float(np.std(dqn_eval_returns)),
    float(np.std(ppo_eval_returns)),
]
fig4.add_trace(
    go.Bar(
        x=["Random", "DQN", "PPO"],
...
fig4.add_trace(
    go.Scatter(
        x=ppo_cumulative_steps, y=ppo_returns, name="PPO", line=dict(color="red")
    ),
    row=1,
    col=2,
)
...
fig4.add_trace(
    go.Table(
        header=dict(values=["Property", "DQN", "PPO"]),
        cells=dict(
            values=[
                [
                    "Type",
...
fig4.update_layout(
    title="RL Algorithm Comparison Dashboard", height=700, showlegend=True
)
