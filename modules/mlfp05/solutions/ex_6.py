        ...
        await ctx.log_params(
            {
                ...
                "n_nodes": str(N),
                "n_edges": str(n_edges_undirected),
            }
        )
...
# Positive: real edges from the graph (use half to avoid duplicates)
    ...
    async with tracker.run(experiment_name=exp_name, run_name="link_prediction") as ctx:
        ...
        for epoch in range(LINK_EPOCHS):
            ...
            with torch.no_grad():
                ...
                # Pairwise comparison: for a random sample of pairs, how often
                # does the positive edge score higher than the negative?
...
async def register_best_models():
    ...
    # Register best node classifier
    best_model_data = results[best_name]
    model_bytes = pickle.dumps(best_model_obj.state_dict())
...
# Plot 1: Node classification — training loss curves
...
# Plot 2: Node classification — validation accuracy curves
...
# Plot 3: Link prediction training
...
print(
  ...
  [x] Registered best model and link predictor in ModelRegistry with
      versioned metrics for reproducible deployment
  ...
  neighbours, but they differ in HOW they aggregate:
    ...
    GraphSAGE  -> sampled + learned (separate self/neighbour transforms)
