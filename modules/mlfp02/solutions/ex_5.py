...
sigma_sq_hat = SSR / (n_obs - k)  # Residual variance (unbiased)
sigma_hat = np.sqrt(sigma_sq_hat)

# Standard errors from diagonal of (X'X)⁻¹
se_beta = np.sqrt(sigma_sq_hat * np.diag(XtX_inv))

# t-statistics and p-values
t_stats = beta_ols / se_beta
p_values = 2 * (1 - stats.t.cdf(np.abs(t_stats), df=n_obs - k))
...
r_squared = 1 - SSR / SST
adj_r_squared = 1 - (1 - r_squared) * (n_obs - 1) / (n_obs - k)

# F-statistic: model vs intercept-only
f_stat = (SSE / (k - 1)) / (SSR / (n_obs - k))
...
# INTERPRETATION: R² of {r_squared:.1%} means the three features explain
# {r_squared:.0%} of the price variation. The F-test confirms the model
# is significantly better than predicting the mean for everyone.
# But {100*(1-r_squared):.0%} of variation is unexplained — location,
# renovation, market timing, etc. matter.
...
# (common for housing data — more expensive flats have more price variation)
abs_resid = np.abs(residuals)
# Fit |residuals| ~ X to get variance model
...
r2_test = 1 - np.sum((y_test - y_test_pred) ** 2) / np.sum(
