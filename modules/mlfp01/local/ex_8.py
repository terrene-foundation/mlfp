...
if "fare" in taxi_clean.columns:
    ...
    # TODO: Use viz.histogram() to plot the fare distribution
    fig_dist = viz.histogram(
        pl.DataFrame({"Fare (S$) — Cleaned": ____}),  # Hint: clean_fares[:50_000]
        "Fare (S$) — Cleaned",
...
if "haversine_km" in taxi_clean.columns:
    ...
    fig_dist_km = viz.histogram(
        pl.DataFrame({"Trip Distance (km)": ____}),  # Hint: distances[:50_000]
        "Trip Distance (km)",
