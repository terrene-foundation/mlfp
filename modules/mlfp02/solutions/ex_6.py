...
for i in range(k_groups):
    for j in range(i + 1, k_groups):
        ...
        q_stat = abs(diff) / se
        # Approximate p-value using studentised range distribution
        # (scipy doesn't have it directly; use Bonferroni approximation)
        ...
        p_bonf = min(p_pair * n_comparisons, 1.0)  # Bonferroni adjustment
        sig = (
            "***"
            if p_bonf < 0.001
