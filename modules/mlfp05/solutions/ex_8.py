...
#
# Epsilon-greedy exploration: with probability epsilon, take a random
# action; otherwise take argmax_a Q(s, a). Epsilon decays from 1.0 to
# a minimum over training, shifting from exploration to exploitation.
...
async def train_ppo_async(
    ...
    async with tracker.run(experiment_name=exp_name, run_name=run_name) as ctx:
        ...
        for it in range(n_iters):
            ...
            await ctx.log_metric("avg_episode_return", avg_ep_return, step=it)
...
# ── Environment 1: Customer Churn Prevention (discrete actions) ──────
...
# ── Environment 2: Dynamic Pricing (continuous-valued state) ─────────
class DynamicPricingEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.state = np.array(
            [
                0.5,  # current price (normalised)
                1.0,  # inventory (starts full)
                self.np_random.uniform(0.3, 0.7),  # demand signal
                self.np_random.uniform(0.3, 0.7),  # competitor price
                1.0,  # time remaining
            ],
            dtype=np.float32,
        )
    ...
    def step(self, action):
        ...
        # Demand model: lower price -> higher demand, also affected by competitor
        ...
        terminated = inventory <= 0.01  # sold out
        truncated = self.step_count >= self.max_steps

        return self.state.copy(), reward, terminated, truncated, {}


# ── Environment 3: Portfolio Rebalancing (multi-discrete) ────────────
class PortfolioRebalancingEnv(gym.Env):
    ...
    Actions (3): For each asset: 0=decrease, 1=hold, 2=increase weight.
                 We use Discrete(27) = 3^3 to enumerate all combos.
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Discrete(27)  # 3^3 combos for 3 assets
        self.max_steps = 24
        self.state = None
        self.step_count = 0
    ...
    def step(self, action):
        ...
        # Volatility penalty (risk-adjusted)
        vol_penalty = market_vol * 0.05 * weights[0]  # stocks carry vol risk
...
# ── Environment 4: Supply Chain Inventory (sequential decisions) ─────
class SupplyChainInventoryEnv(gym.Env):
    ...
    def step(self, action):
        ...
        seasonal_factor = 0.5 + 0.3 * np.sin(2 * np.pi * season)  # peak in winter
        ...
        # Update forecast (noisy signal of next demand)
        forecast = np.clip(
...
# ── Environment 5: Resource Allocation (constrained optimisation) ────
class ResourceAllocationEnv(gym.Env):
    ...
    Actions (6): 0-5 = shift one unit from service X to service Y (6 pairs)
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Discrete(7)  # 6 shifts + do_nothing
        self.max_steps = 48
        self.state = None
        self.step_count = 0
    ...
    def step(self, action):
        ...
        # Apply reallocation (shift 0.05 between services)
        ...
        # Simulate load changes (time-of-day pattern)
        hour = (self.step_count % 24) / 24.0
        diurnal = 0.3 + 0.5 * np.sin(np.pi * hour)  # peak at noon
        for i in range(3):
            loads[i] = np.clip(
        ...
        # SLA check: service meets SLA if alloc >= load * 0.8
        sla_met = 0
        sla_violations = 0
        for i in range(3):
...
# We train on ChurnPrevention (4-D state, 4 actions) and register the
# trained policy network as a versioned artifact.
...
# Register trained policy in ModelRegistry
async def register_rl_models():
    ...
    # Register DQN CartPole
    dqn_bytes = pickle.dumps(dqn_model.state_dict())
    dqn_version = await registry.register_model(
        ...
        metrics=[
            ...
            MetricSpec(name="algorithm", value=0.0),  # DQN tag
...
# ── Plot 1: DQN training on CartPole ─────────────────────────────────
...
# ── Plot 2: PPO training on CartPole ─────────────────────────────────
fig2 = viz.training_history(
    metrics={
        "PPO avg episode return": ppo_returns,
    },
...
# ── Plot 3: Policy comparison bar chart ──────────────────────────────
# Compare random vs DQN vs PPO evaluation returns
...
# ── Plot 4: DQN on ChurnPrevention ──────────────────────────────────
fig4 = viz.training_history(
    metrics={
        "ChurnPrevention DQN reward": churn_rewards_hist,
        "ChurnPrevention moving avg (20)": moving_average(churn_rewards_hist, 20),
    },
    x_label="Episode",
    y_label="Reward",
)
...
fig5 = viz.training_history(
    metrics={
        "DQN loss (CartPole)": [l for l in dqn_losses if l > 0],
    },
    x_label="Episode",
    y_label="Loss",
)
