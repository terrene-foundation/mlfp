...
async def setup_engines():
    ...
    exp_name = await tracker.create_experiment(
        name="m5_gans", description="DCGAN vs WGAN-GP on MNIST (60K images)"
    )
    try:
...
# The adversarial training concepts (BCE, Wasserstein, gradient penalty,
# mode collapse) are identical to conv-based architectures.
LATENT_DIM = 64
IMG_DIM = 28 * 28
...
async def train_vanilla_gan_async(epochs: int = 15, lr: float = 2e-4):
    """Train a vanilla GAN; log to tracker via the async context manager."""
...
# L_G      = -E[D(fake)]
# GP (Gulrajani 2017): interpolate between real and fake, compute grad
# of D, penalise ||grad||_2 deviating from 1. Replaces weight clipping
# which caused capacity underuse (weights cluster at clip boundary).
...
async def train_wgan_gp_async(
    ...
    """Train WGAN-GP; log to tracker via the async context manager."""
    G, D = Generator().to(device), Discriminator().to(device)
...
#
# We use a LeNet feature extractor trained on MNIST rather than InceptionV3
# (designed for 299x299 RGB ImageNet). The FID formula is identical — only
# the feature space changes. For 28x28 grayscale, a domain-specific
# extractor gives more meaningful distance measurements.
...
# Train the feature extractor on MNIST
...
# Compute FID for both generators
...
# Log FID to tracker (async context manager — auto end_run on exit)
async def _log_fid_async():
    async with tracker.run(experiment_name=exp_name, run_name="fid_evaluation") as ctx:
        ...
        await ctx.log_metrics({"fid_vanilla_gan": fid_gan, "fid_wgan_gp": fid_wgan})
...
# We classify generated samples and check all 10 classes are covered.
# Shannon entropy quantifies diversity: max = log2(10) = 3.32 (uniform).
...
async def _log_mode_coverage_async():
    async with tracker.run(experiment_name=exp_name, run_name="mode_coverage") as ctx:
        await ctx.log_metrics(
            {
                "gan_coverage": float(cov_gan),
                "gan_entropy": ent_gan,
                "wgan_coverage": float(cov_wgan),
                "wgan_entropy": ent_wgan,
            }
        )
...
fig_wgan = viz.training_history(
    metrics={"WGAN-GP G loss": wgan_g_losses, "WGAN-GP D loss": wgan_d_losses},
    x_label="Epoch",
    y_label="Loss",
)
