...
| 2.2 | **Estimation and Inference**                | MLE derivation (practical: write log-likelihood, optimize, interpret), MAP as MLE-with-prior, when MLE fails (small n, multimodal, misspecified)                           | Workflow: optimization → visualization                                          | NEW: MLE for Singapore economic parameters       |
...
| 2.5 | **CUPED and Variance Reduction**            | SRM detection, CUPED (derive Var(Y_adj) = Var(Y)(1-ρ²)) — the single most important A/B technique. Focus on implementation + intuition                                     | `ExperimentTracker` full lifecycle                                              | Existing M2 ex_3: CUPED implementation           |
| 2.6 | **Sequential Testing and Causal Inference** | Sequential testing (alpha spending), Bayesian A/B (optional enrichment). DiD (derive ATT), propensity matching, parallel trends, placebo tests                             | `ExperimentTracker` for causal logging                                          | Existing M2 ex_4: DiD on cooling measures        |
| 2.7 | **Feature Engineering**                     | Temporal features, interaction terms, polynomial features, leakage detection, feature selection strategies                                                                 | `FeatureEngineer` (generate + select), `FeatureSchema`, `FeatureField`          | Existing M2 ex_1: Feature engineering pipeline   |
| 2.8 | **Feature Store and Project**               | Point-in-time correctness, data lineage, `FeatureStore` lifecycle. Then: capstone project (design → execute → analyze → report, 3-4 options)                               | `FeatureStore`, `ExperimentTracker`, `ModelVisualizer`                          | Existing M2 ex_2 + ex_5: Store + project         |
