...
# ── Train both models ─────────────────────────────────────────────────
# Transfer learning trains only a 5K-parameter head on frozen ResNet
# features; the scratch CNN trains all ~180K parameters. Both get the
# same epoch budget for a fair comparison.
EPOCHS = 8
...
async def register_and_compare():
    ...
    if has_registry:
        ...
        transfer_bytes = pickle.dumps(transfer_model.state_dict())
        transfer_version = await registry.register_model(
            name="cifar10_resnet18_transfer",
            artifact=transfer_bytes,
            metrics=[
        ...
        # Register scratch model
        scratch_bytes = pickle.dumps(scratch_model.state_dict())
        scratch_version = await registry.register_model(
            name="cifar10_cnn_scratch",
            artifact=scratch_bytes,
            metrics=[
...
def extract_features(model: nn.Module, loader: DataLoader, max_samples: int = 2000):
    ...
    # Hook into the avgpool layer to capture features before fc
    hook_features = []
    ...
    if hasattr(model, "avgpool"):
        ...
        # For Sequential models, use the second-to-last layer output
...
# Run t-SNE (perplexity=30 is standard; n_iter=500 for speed)
tsne_transfer = TSNE(n_components=2, perplexity=30, n_iter=500, random_state=42)
coords_transfer = tsne_transfer.fit_transform(transfer_feats)

tsne_scratch = TSNE(n_components=2, perplexity=30, n_iter=500, random_state=42)
coords_scratch = tsne_scratch.fit_transform(scratch_feats)


# Print cluster quality: average intra-class distance / inter-class distance
...
# Print a few class centroids for each
...
async def serve_predictions():
    ...
    # Register the PyTorch model (not ONNX, since InferenceServer wraps it)
    ...
    try:
        ...
        # Get a batch of test images for prediction
        test_batch_x, test_batch_y = next(iter(val_loader))

        # Predict on the batch (first 8 images)
        ...
        # Still demonstrate predictions directly
        with torch.no_grad():
...
#
# Adapter comparison:
#   Full fine-tuning:  Update ALL params     (100% trainable, risk of forgetting)
#   Frozen head:       Update ONLY fc head   (~0.05% trainable, limited capacity)
#   Bottleneck adapter: Insert trainable layers (~5-10% trainable, good balance)
#   LoRA (M6):         Low-rank weight updates (~1-5% trainable, best for LLMs)
...
def build_adapter_resnet(n_classes: int = N_CLASSES, bottleneck: int = 64) -> nn.Module:
    ...
    # We wrap the existing layers with adapter modules
    ...
    class AdaptedBlock(nn.Module):
        ...
        def forward(self, x):
            out = self.block(x)
            # Adapter operates on channel-wise pooled features
            b, c, h, w = out.shape
...
# Train for 3 epochs (concept demo, not full training)
...
print(
    f"""
  [x] Loaded a real pre-trained torchvision.models.resnet18 (ImageNet weights)
  [x] Froze the backbone and trained only a new classifier head on CIFAR-10
  [x] Compared transfer learning vs from-scratch CNN on FULL CIFAR-10 (50K)
  [x] Logged both runs to ExperimentTracker with per-epoch loss and accuracy
  [x] Registered both models in ModelRegistry with metric-based promotion
  [x] Data efficiency experiment: {efficiency_results.get(0.10, 0):.1%} acc with 10% data,
      {efficiency_results.get(1.0, 0):.1%} acc with 100% data
  [x] Visualised learned features with t-SNE (cluster quality: transfer
      {cq_transfer:.3f} vs scratch {cq_scratch:.3f})
  [x] Exported to ONNX and served predictions with InferenceServer
  [x] Built a bottleneck adapter module ({n_adapter_trainable:,} trainable params,
      {adapter_acc:.1%} accuracy) — preview of M6 LoRA
