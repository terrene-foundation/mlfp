...
"""
# ════════════════════════════════════════════════════════════════════════
# MLFP05 — Exercise 3: Word Embeddings
# ════════════════════════════════════════════════════════════════════════
    ...
    for i, token in enumerate(tokens):
        ...
        for j in range(start, end):
            if j == i or tokens[j] not in word_to_idx:
                continue
            pairs.append((center_idx, word_to_idx[tokens[j]]))
    return pairs
...
def train_skipgram(
    ...
    for epoch in range(epochs):
        ...
        for center, context in pairs[:5000]:  # Limit for speed
            # Positive pair
            score = dot_product(W_center[center], W_context[context])
            prob = sigmoid(score)
...
def analogy(a: str, b: str, c: str, top_k: int = 3) -> list[tuple[str, float]]:
    ...
    if any(w not in word_to_idx for w in [a, b, c]):
        return []
    vec = [
        W_center[word_to_idx[b]][d]
        - W_center[word_to_idx[a]][d]
        + W_center[word_to_idx[c]][d]
        for d in range(embedding_dim)
    ]
    exclude = {a, b, c}
    sims = []
