...
if HAS_TORCH:
    ...
    dropout_layer.train()
    dropped = dropout_layer(test_input)
    n_zeros_train = (dropped == 0).sum().item()

    dropout_layer.eval()
    not_dropped = dropout_layer(test_input)
...
"""
    ...
    fig_lr = viz.training_history({"Learning Rate": history["lr"]}, x_label="Epoch")
    ...
    fig_grad = viz.training_history(
        {"Gradient Norm": history["grad_norm"]}, x_label="Epoch"
    )
