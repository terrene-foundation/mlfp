...
def precision_at_k(
    ...
    for u in range(predictions.shape[0]):
        ...
        if len(holdout_items) == 0:
            continue
        # Relevant = items the user rated >= threshold in holdout
        relevant = set(j for j in holdout_items if R_true[u, j] >= threshold)
        if len(relevant) == 0:
            continue
        # Top-k predictions (among unobserved training items)
        unobserved = ~holdout[u]  # Items NOT in holdout (could recommend)
        pred_scores = predictions[u].copy()
        pred_scores[holdout[u]] = -np.inf  # Don't recommend holdout items
        pred_scores[np.isnan(pred_scores)] = -np.inf
        top_k_items = np.argsort(pred_scores)[-k:]
        # But evaluate against holdout
        # Use a different approach: rank holdout items by predicted score
        holdout_pred = [
            (j, predictions[u, j])
...
# Normalise each method's predictions to [0, 1] for blending
...
# Weight by MAP score (better methods get higher weight)
map_scores = {name: res["MAP"] for name, res in eval_results.items()}
total_map = sum(max(m, 0.01) for m in map_scores.values())
weights = {name: max(m, 0.01) / total_map for name, m in map_scores.items()}
