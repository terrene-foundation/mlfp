...
#   We re-use the data loading from _shared.py.
#
# PHASES:
...
class SEBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        b, c, _, _ = x.shape
        # Squeeze: spatial (H,W) -> single number per channel
        s = F.adaptive_avg_pool2d(x, 1).view(b, c)
        # Excite: learn channel importance weights
        w = self.fc(s).view(b, c, 1, 1)
        # Scale: amplify important channels, suppress irrelevant ones
        return x * w
