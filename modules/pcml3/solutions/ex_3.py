...
print(f"  Additivity check: {'✓ PASS' if abs(shap_sum - model_output) < 0.01 else '✗ FAIL'}")
...
for feat in top_features:
    ...
    corr = np.corrcoef(feat_vals[~np.isnan(feat_vals)], feat_shap[~np.isnan(feat_vals)])[0, 1]
...
if protected_in_model:
    ...
    for attr in protected_in_model:
        ...
        print(f"    Rank: #{[n for n, _ in importance_ranking].index(attr) + 1} / {len(feature_names)}")
        ...
        if len(unique_vals) <= 10:
            for val in sorted(unique_vals):
                ...
                print(f"    Value={val:.0f}: mean SHAP = {mean_shap:+.4f} (n={mask.sum()})")
