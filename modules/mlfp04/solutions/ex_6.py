...
# ── Checkpoint 2 ─────────────────────────────────────────────────────
# BM25 should produce positive scores for terms present in documents
test_bm25 = bm25_score(tf=3, df=2, N=10, dl=50, avgdl=40)
assert test_bm25 > 0, "BM25 score should be positive for present terms"
# Saturation: doubling TF should less than double the score
...
n_nmf_topics = 3
nmf_toy = NMF(n_components=n_nmf_topics, random_state=42, max_iter=300)
W_toy = nmf_toy.fit_transform(X_tfidf)
...
# Simple lexicon-based sentiment (production systems use transformers)
positive_words = {
    "good",
    "great",
    "excellent",
