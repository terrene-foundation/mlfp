...
X_drifted_sudden[:, 0] = rng.normal(
    loc=X_train[:, 0].mean() + 3 * X_train[:, 0].std(),
    scale=X_train[:, 0].std(),
    size=X_test.shape[0],
)

print("\nScenario 2: Sudden drift (3σ mean shift on feature 0)")
psi_sudden = compute_psi(X_train[:, 0], X_drifted_sudden[:, 0])
ks_stat_sudden, ks_pval_sudden = compute_ks(X_train[:, 0], X_drifted_sudden[:, 0])
