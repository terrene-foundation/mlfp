...
def scaled_dot_product_attention(
    ...
    # einsum "bqd,bkd->bqk" means: for each batch b, multiply the
    # query at position q with every key at position k across dimension d.
    ...
    # is nearly zero. This is the "scaled" in "scaled dot-product attention."
    ...
    # Step 4: Softmax over the key dimension. This converts raw scores
    # into a probability distribution: how much should each query attend
    # to each key?
    weights = F.softmax(scores, dim=-1)

    # Step 5: Weighted sum of values. Each query's output is a weighted
    # average of all values, where the weights come from the attention scores.
...
# Create a simple learned embedding + attention to show realistic patterns
embed_dim = 32
embedding = torch.nn.Embedding(len(vocab), embed_dim, padding_idx=0)
with torch.no_grad():
    embedded = embedding(sample_indices)  # (1, MAX_LEN, embed_dim)
    # Use the embeddings as Q, K, V (self-attention)
    _, sample_attn = scaled_dot_product_attention(embedded, embedded, embedded)
...
# embeddings fine-tuned on Singapore case law.
query_texts = [
...
# Compute attention-weighted representations for query documents
...
def get_attention_representation(text: str) -> torch.Tensor:
    ...
    with torch.no_grad():
        emb = embedding_legal(indices)  # (1, MAX_LEN, embed_dim)
        attn_out, _ = scaled_dot_product_attention(emb, emb, emb)
        # Mean pool over non-pad positions
        mask = (indices != 0).float().unsqueeze(-1)  # (1, MAX_LEN, 1)
...
for qi, (q_text, q_label) in enumerate(zip(query_texts, query_labels)):
    q_rep = get_attention_representation(q_text)
    # Cosine similarity against all candidates
    similarities = F.cosine_similarity(q_rep.unsqueeze(0), candidate_reps, dim=1)
