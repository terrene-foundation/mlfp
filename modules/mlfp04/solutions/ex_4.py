...
# Compute Z-scores per feature (X_scaled is already standardised)
z_scores = np.abs(X_scaled)

# Anomaly score: maximum Z-score across all features
...
for threshold in [2.0, 2.5, 3.0, 3.5]:
    ...
    # Among flagged, what fraction are true anomalies?
...
# Winsorisation: clip extreme values to bounds
X_winsorised = np.clip(X_scaled, lower_bound, upper_bound)
...
# Best contamination: close to actual anomaly rate
...
ensemble_equal = (z_norm + iqr_norm + iso_norm + lof_norm) / 4.0
...
# At least one ensemble should match or beat the best single method
